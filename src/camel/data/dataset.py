...
class TabularDataset:
    ...
    def subsample(
        ...
        """Create a subsample of the dataset using various sampling strategies.

        This method supports multiple sampling modes including random, stratified,
        uniform, and fixed sampling. It maintains the original dataset structure
        while creating a smaller representative sample.

        Args:
            subsample_mode (str): Sampling strategy to use. Options:
                - 'random': Random sampling without stratification
                - 'stratified': Maintains class distribution proportions
                - 'uniform': Equal samples per class
                - 'customised_ratio': Custom class distribution
                - 'fixed': Use pre-specified indices
            subsample_size (Optional[int | float], optional): Number of samples
                to include. If float (0-1), treated as proportion. Required for
                non-fixed sampling modes. Defaults to None.
            subsample_indices (Optional[list[int]], optional): Specific indices
                to include. Only used with 'fixed' mode. Defaults to None.
            customised_class2distribution (Optional[dict], optional): Custom class
                distribution for 'customised_ratio' mode. Keys are class labels,
                values are desired proportions. Defaults to None.
            random_state (Optional[int], optional): Random seed for reproducibility.
                Defaults to 42.

        Returns:
            dict: Dictionary containing:
                - 'dataset_subsample': New TabularDataset with subsampled data
                - 'subsample_indices': List of original indices included in subsample

        Raises:
            ValueError: If required parameters are missing for the chosen mode
            ValueError: If subsample_size exceeds dataset size
            ValueError: If stratified sampling requested with insufficient samples

        Example:
            >>> # Stratified subsampling maintaining class proportions
            >>> result = dataset.subsample('stratified', subsample_size=1000)
            >>> subsample_dataset = result['dataset_subsample']
            >>>
            >>> # Custom class distribution
            >>> custom_dist = {'class_A': 0.6, 'class_B': 0.4}
            >>> result = dataset.subsample('customised_ratio',
            ...                          subsample_size=500,
